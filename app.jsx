...
const LAYER_ICONS = {
  base: "👕", outer: "🧥", bottom: "👖", accessories: "🧤", footwear: "👟"
};

const LAYER_LABELS = {
  base: "Base Layer", outer: "Outer Layer", bottom: "Bottoms", accessories: "Accessories", footwear: "Footwear"
};
...
function phase4Recommend(weather, activity, duration, logs) {
  ...
  const scored = justRightLogs.map(l => ({
    log: l,
    dist: euclideanDist(
      { feelsLike: weather.feelsLike, humidity: weather.humidity, windSpeed: weather.windSpeed },
      { feelsLike: l.weather.feelsLike, humidity: l.weather.humidity, windSpeed: l.weather.windSpeed }
    )
...
function generateDemoLogs(count = 25) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const outfit = phase1Recommend(effTemp, Math.random() > 0.7, Math.random() > 0.9);
    logs.push({
      id: `demo-${i}-${Date.now()}`,
      timestamp: Date.now() - (count - i) * 86400000,
      weather: { temp, feelsLike, humidity: Math.round(40 + Math.random() * 50), windSpeed: Math.round(Math.random() * 30), precipitation: Math.random() > 0.7 ? +(Math.random() * 5).toFixed(1) : 0, uvIndex: Math.round(Math.random() * 10), weatherCode: [0, 1, 2, 3, 51, 61, 71][Math.floor(Math.random() * 7)], location: "Toronto, ON" },
      activity,
...
export default function LayerUp() {
  ...
  useEffect(() => {
    try { localStorage.setItem("layerup_logs_v2", JSON.stringify(logs)); } catch {}
  }, [logs]);
  
  useEffect(() => {
    try { localStorage.setItem("layerup_unit", unit); } catch {}
  }, [unit]);
    ...
    async function fetchWeather() {
      ...
      try {
        ...
        try {
          ...
          console.warn("Geolocation failed/timed out. Using default location.", geoErr);
        }
    ...
    <div className="app">
              ...
              <div className="stat-grid">
                ...
                <div className="stat-card">
                  <div className="stat-val" style={{ color: "var(--green)" }}>
                    {Math.round((logs.filter(l => l.comfort === "justRight").length / logs.length) * 100)}%
                  </div>
                  <div className="stat-label">Just Right</div>
                </div>
                <div className="stat-card">
                  <div className="stat-val">
                    {calculateOffset(logs) > 0 ? "❄️" : calculateOffset(logs) < 0 ? "🔥" : "⚖️"}
                  </div>
                  <div className="stat-label">{calculateOffset(logs) > 0 ? "Runs Cold" : calculateOffset(logs) < 0 ? "Runs Warm" : "Neutral"}</div>
                </div>
                <div className="stat-card">
                  ...
                  {ACTIVITY_LEVELS.map(a => {
                    ...
                    return (
                      <div key={a.key} className="bar-row">
                        ...
                        <div className="bar-track">
                          <div className="bar-fill" style={{ width: `${Math.max(normalized * 100, 8)}%`, background: offset > 0 ? "var(--cold)" : offset < 0 ? "var(--warm)" : "var(--green)" }}>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                  {[1, 2, 3, 4].map(p => (
                    <div key={p} style={{
                      ...
                      background: phase >= p ? "rgba(91, 159, 224, 0.15)" : "var(--surface2)",
                      border: `1px solid ${phase >= p ? "var(--accent)" : "var(--border)"}`,
                      textAlign: "center", opacity: phase >= p ? 1 : 0.4,
                      transition: "all 0.3s"
                    }}>
                      <div style={{ fontSize: 18, fontWeight: 700, color: phase >= p ? "var(--accent)" : "inherit" }}>{p}</div>
                      <div style={{ fontSize: 10, fontWeight: 500, color: phase >= p ? "var(--text)" : "var(--text3)", marginTop: 4 }}>
                        {p === 1 ? "Baseline" : p === 2 ? `10 logs` : p === 3 ? `30 logs` : `50 logs`}
                      </div>
                    </div>
                  ))}
                </div>
      ...
      {tab === "settings" && (
        <div className="settings-page">
          ...
          <button className="demo-btn" onClick={() => { setLogs(prev => [...prev, ...generateDemoLogs(25)]); }}>
      ...
      {showLog && recommendation && (
        <div className="sheet-overlay" onClick={e => { if (e.target === e.currentTarget) setShowLog(false); }}>
          <div className="sheet">
            ...
            <div className="selector-section" style={{ padding: 0, marginBottom: 20 }}>
              ...
              <div className="outfit-layers">
                {logOutfit && Object.keys(LAYERS).map(cat => {
                  ...
                  const options = LAYERS[cat];
                  return (
                    <div key={cat} className="outfit-layer" style={{ cursor: "pointer" }}
                      onClick={() => {
                        setLogOutfit(prev => ({ ...prev, [cat]: (prev[cat] + 1) % options.length }));
                      }}>
                      <div className="layer-icon">{LAYER_ICONS[cat]}</div>
                      <div className="layer-info">
                        <div className="layer-cat">{LAYER_LABELS[cat]}</div>
            ...
            <button className="sheet-submit" disabled={!logComfort} onClick={submitLog}>
              Save Log
            </button>
          </div>
        </div>
