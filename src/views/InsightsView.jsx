...
export function InsightsView({ logs, phase, calculateOffset, displayOffset }) {
  ...
  const comfortRate = logs.length
    ? Math.round(
        (logs.filter((log) => log.comfort === 'justRight').length /
          logs.length) *
          100
      )
    : 0;
  const thermalProfile =
    overallOffset > 0
      ...
      : overallOffset < 0
        ...
        : 'Neutral';
  const thermalSymbol =
    overallOffset > 0 ? '❄️' : overallOffset < 0 ? '🔥' : '◌';
    ...
    <main className="page insights-page">
      ...
      <section className="page-intro">
        ...
        <p className="page-copy">
          This is the app&apos;s current read on how your comfort shifts with
          weather and effort.
        </p>
      </section>
      ...
      {logs.length === 0 ? (
        <section className="content-panel empty-state">
          ...
          <p className="empty-copy">
            Add a few real-world logs to unlock your comfort patterns and
            activity offsets.
          </p>
        </section>
      ) : (
            ...
            <div className="bar-chart">
              {ACTIVITY_LEVELS.map((activity) => {
                const activityLogs = logs.filter(
                  (log) => log.activity === activity.key
                );
                const offset =
                  activityLogs.length >= 3 ? calculateOffset(activityLogs) : 0;
          ...
          <section className="content-panel insight-section">
            <div className="panel-heading">
              ...
              <p className="panel-copy">
                Phase 4 only unlocks once LayerUp can match current conditions
                against similar successful days.
              </p>
            </div>
            ...
            <div className="phase-grid">
              {[1, 2, 3, 4].map((value) => (
                <div
                  key={value}
                  className={`phase-step ${phase >= value ? 'active' : ''} ${phase === value ? 'current' : ''}`}
                >
